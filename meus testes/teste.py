...
def formatedData():
    ...
    filteredData = [row.split(';') for row in data]
    return filteredData

def createCsv(data):
    df = pd.DataFrame(data, columns=[
        'CNPJ_BASICO', 'CNPJ_ORDEM', 'CNPJ_DV', 'IDENTIFICADOR_MATRIZ_FILIAL',
        'NOME_FANTASIA', 'DATA_SITUACAO_CADASTRAL', 'MOTIVO_SITUACAO_CADASTRAL',
        ...
        'MUNICIPIO', 'DDD1', 'TELEFONE1', 'DDD2', 'TELEFONE2', 'DDD_DO_FAX',
        'FAX', 'CORREIO_ELETRONICO', 'SITUACAO_ESPECIAL',
        'DATA_DA_SITUACAO_ESPECIAL', 'extra', 'extra2'])
    label = 'csv.csv'
    df.to_csv(label)
    return label
